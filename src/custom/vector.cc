...
void Vector::tick()
{
  ...
  if (!outPipeStall)
  {
    if (pipeSrc == Pipeline)
    {
      ...
      if (!hasNextStage())
      { // actually make sure its the last stage that does this (HACK)
...
void Vector::linetrace(std::stringstream &ss)
{
  // TODO this can be really useful in debugging!
}

void Vector::doSquash(SquashComm::BaseSquash &squashInfo, StageIdx initiator)
{
...
void Vector::forwardInstruction(const MasterData &instInfo)
{
...
void Vector::pullPipeInstruction(MasterData &instInfo)
{
...
void Vector::pullMeshInstruction(MasterData &instInfo)
{
...
void Vector::pushPipeInstToNextStage(const MasterData &instInfo)
{
...
void Vector::pushMeshInstToNextStage(const MasterData &instInfo)
{
...
bool Vector::getOutRdy()
{
  ...
  for (int i = 0; i < getMeshMasterPorts().size(); i++)
  {
    ...
    {
      if (!getMeshMasterPorts()[i].getPairRdy())
        allRdy = false;
    }
  }
...
bool Vector::getInVal()
{
  ...
  for (int i = 0; i < getMeshSlavePorts().size(); i++)
  {
    ...
    {
      if (!getMeshSlavePorts()[i].getPairVal())
        allVal = false;
...
void Vector::resetActive()
{
  ...
  if (!getConfigured() || canWriteMesh())
  {
    ...
    for (int i = 0; i < getMeshMasterPorts().size(); i++)
    {
      ...
      if (!getConfigured())
        getMeshMasterPorts()[i].setDriver(nullptr);
      else
        getMeshMasterPorts()[i].setDriver(this);
    }
  }
  ...
  if (!getConfigured() || canReadMesh())
  {
    ...
    for (int i = 0; i < getMeshSlavePorts().size(); i++)
    {
      ...
      // set the stage driving the ports (effectively setting mux select)
      if (!getConfigured())
        getMeshSlavePorts()[i].setDriver(nullptr);
      else
        getMeshSlavePorts()[i].setDriver(this);
    }
  }
}

bool Vector::isRootMaster()
{
  return isMaster() && !isSlave();
}

bool Vector::isMaster()
{
...
bool Vector::canReadMesh()
{
  return (_canRecv && isSlave());
}

bool Vector::isOutPipeStalled()
{
  return checkStall();
}

bool Vector::isInPipeStalled()
{
  return m_insts.empty();
}

bool Vector::isOutMeshStalled()
{
  return getConfigured() && !getOutRdy();
}

bool Vector::isInMeshStalled()
{
  return getConfigured() && (!getInVal() || getRevecStall());
}

bool Vector::getRevecStall()
{
  return (meshHasRevec() && !pipeHasRevec());
}

// inform that there has been an update
void Vector::neighborUpdate()
{
...
bool Vector::pipeHasRevec()
{
  return _pipeRevecId >= 0;
}

bool Vector::meshHasRevec()
{
  return _meshRevecId >= 0;
}

int Vector::getPipeRevec()
{
  return _pipeRevecId;
}

int Vector::getMeshRevec()
{
  return _meshRevecId;
}

void Vector::setPipeRevec(int val)
{
  _pipeRevecId = val;
}

void Vector::setMeshRevec(int val)
{
  _meshRevecId = val;
}

void Vector::resetPipeRevec()
{
  _pipeRevecId = -1;
}

void Vector::resetMeshRevec()
{
  _meshRevecId = -1;
}
