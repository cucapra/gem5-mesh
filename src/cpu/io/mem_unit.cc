...
void MemUnit::setScoreboardPtr(Scoreboard *scoreboard_p)
{
  m_scoreboard_p = scoreboard_p;
}

void MemUnit::insert(IODynInstPtr inst)
{
  assert(!isBusy());
...
void MemUnit::functionalExecute()
{
...
bool MemUnit::hasInstsToWriteBack() const
{
...
void MemUnit::tryLdIssue(size_t &num_issued_insts)
{
  for (auto &inst : m_ld_queue)
  {
        ...
        inst->canIssueToMem())
    {
      ...
      // send request
      DPRINTF(Mesh, "Packet address mesh unit %#x\n", pkt->getAddr());
...
void MemUnit::tryStIssue(size_t &num_issued_insts)
{
...
void MemUnit::doSquash(IODynInstPtr squash_inst)
{
...
void MemUnit::processCacheCompletion(PacketPtr pkt)
{
  // extract SenderState
  ...
  if (ss->inst->isLoad())
  {
    ...
    auto it = std::find_if(m_ld_queue.begin(),
                           m_ld_queue.end(),
                           [&](const IODynInstPtr &inst) { return ss->inst->seq_num == inst->seq_num; });
                          ...
                          uint64_t *res, AtomicOpFunctor *amo_op)
{
  ...
  if ((addr & (line_size - 1)) + size > line_size)
  {
    ...
    if (is_load)
    {
      m_s1_inst->fault = std::make_shared<RiscvISA::AddressFault>(addr, RiscvISA::LOAD_ADDR_MISALIGNED);
    }
    else if (amo_op)
    {
      m_s1_inst->fault = std::make_shared<RiscvISA::AddressFault>(addr, RiscvISA::AMO_ADDR_MISALIGNED);
    }
    else
    {
      m_s1_inst->fault = std::make_shared<RiscvISA::AddressFault>(addr, RiscvISA::STORE_ADDR_MISALIGNED);
    }
...
void MemUnit::finishTranslation(const Fault &fault, RequestPtr req)
{
  assert(m_s1_inst);
  if (fault == NoFault)
...
bool MemUnit::checkLdStDependency(IODynInstPtr ld_inst)
{
...
void MemUnit::linetrace(std::stringstream &ss)
{
#ifdef DEBUG
