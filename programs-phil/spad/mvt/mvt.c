...
mvt_main(int used, int is_da, int mask, DTYPE *a, DTYPE *y1, DTYPE *y2, DTYPE *x1, DTYPE *x2, int n, 
  ...
  // if(ptid==0)printf("1. ptid pointer %x, ptid cores %d %d %d %d\n",ptid_group,ptid_group[0],ptid_group[1],ptid_group[2],ptid_group[3]);
  #ifdef STACK_COPY
  unsigned long long *spTop = getSpTop(ptid);
  // // guess the remaining of the part of the frame (n) that might be needed?? here n = 30
  // if(ptid==0)printf("1. sptop %x\n",spTop);
  spTop -= 40;
  // if(ptid==0)printf("2. sptop %x\n",spTop);
  ...
  reduce_parallel(x2_partial, x2, n, ptid, pdim, unique_id, total_groups, vtid,
...
// linearize some fields
  ...
  #ifndef _VEC
  int spmOffset = TOKEN_LEN; //Sp offset for tokens
  #else
  int spmOffset = REGION_SIZE*NUM_REGIONS+VEC_LEN + 10; //keep it in non region area
  #endif
  int bufSize = 4;
